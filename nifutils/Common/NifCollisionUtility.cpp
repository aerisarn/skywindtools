...
unsigned int NifCollisionUtility::addCollision(string fileNameCollSrc, string fileNameNifDst, string fileNameCollTmpl)
{
	...
	ifstream	iStream(fileNameCollSrc.c_str());
	//  return existance of file
	if(!iStream.good()) {
		logMessage(NCU_MSG_TYPE_ERROR, "Missing Coll src");
		return NCU_ERROR_MISSING_FILE_NAME;
	}

	ifstream	tStream(fileNameCollTmpl.c_str());
	//  return existance of file
	if(!tStream.good()) {
		logMessage(NCU_MSG_TYPE_ERROR, "Missing Coll templ");
		return NCU_ERROR_MISSING_FILE_NAME;
	}

	ifstream	dStream(fileNameNifDst.c_str());
	//  return existance of file
	if(!dStream.good()) {
		logMessage(NCU_MSG_TYPE_ERROR, "Missing Coll dst");
		return NCU_ERROR_MISSING_FILE_NAME;
	}
